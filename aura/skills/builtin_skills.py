...
def build_builtin_skill_registry() -> SkillRegistry:
    ...
    registry.register(
        AuraSkill(
            ...
            description="Review the local chat alpha chain across CLI session, controlled message store, persona response, model boundary, permission gate, safety/uncertainty, and history viewer without opening model, network, memory, command, file, desktop, voice, vision, or full chat runtime.",
            role="companion",
            permission_action="read_project",
            status="online",
            capabilities=[
                "local_chat_integration_review_status",
                "local_chat_integration_alpha",
    ...
    registry.register(
        AuraSkill(
            ...
            description="Persist one safe local chat CLI turn to AURA's controlled message store without model runtime, memory runtime, command execution, arbitrary file mutation, desktop control, voice, vision, or network access.",
            role="companion",
            permission_action="user_confirmation",
            status="online",
            capabilities=[
                "local_chat_message_store_alpha",
                "controlled_jsonl_store_write",
    ...
    registry.register(
        AuraSkill(
            ...
            description="Prepare review-only Sprint 121-130 stabilization checkpoint without activating runtime, opening runtime gates, mutating capability registry, changing permissions, writing audit events, emitting dashboard events, dispatching actions, executing tools/commands, using file runtime, starting services, probing network, performing ORION handshakes, writing memory, or performing git runtime.",
            role="partner",
            permission_action="read_project",
            status="online",
            capabilities=[
