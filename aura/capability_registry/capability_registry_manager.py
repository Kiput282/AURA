...
class CapabilityRegistryManager:
    ...
    {'id': 'permission_audit_action_visibility_ux',
     ...
     'category': 'memory_runtime',
     'control_center_visible': True},
    {'id': 'daily_use_acceptance_rehearsal_release_harness',
     'name': 'Daily-use Acceptance Rehearsal Release Harness',
     'description': 'Permission-gated local browser review runtime for explicitly selected '
                    'user-message memory candidates. Candidates live only in an in-process queue and '
                    'support edit, privacy hold, importance and pinning preview, transient rejection, '
                    'and approval of a future permission envelope preview. Candidate persistence, '
    ...
    {'id': 'daily_local_assistant_live_acceptance_stabilization',
     ...
     'control_center_visible': True},
    {
        "id": "voice_daily_use_activation",
        "name": "Voice Daily-Use Activation",
        "description": (
            "Permission-gated localhost browser voice runtime with "
            "explicit ORION push-to-talk capture, bounded WAV upload "
            "to ATLAS, isolated local faster-whisper STT, editable "
            "transcript preview, fragment-only chat-draft handoff "
            "without automatic send, explicit local Piper TTS, and "
            "ORION playback and stop controls. Microphone access is "
            "self-only and media is restricted to self/blob. "
            "Always-listening, wake word, automatic chat send, TTS "
            "autoplay, cloud fallback, raw-audio retention, voice "
            "action dispatch, and automatic sensitive-action "
