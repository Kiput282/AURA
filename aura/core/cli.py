...
from aura.permission_expiry_recovery_review.aura_permission_expiry_recovery_review_cli import handle_permission_expiry_recovery_review_command
from aura.backup_restore_rehearsal.aura_backup_restore_rehearsal_cli import handle_backup_restore_rehearsal_command
...
from aura.model_loading_unloading_queue_resource_budgets.model_lifecycle_queue_budget_cli import handle_model_lifecycle_queue_budget_command
...
class AuraCLI:
    ...
    def handle_partner_runtime_cli_command(self, raw_args: list[str]) -> bool:
        ...
        if raw_args and raw_args[0] in sprint_240_commands:
            from aura.partner_runtime import (
        ...
        # Sprint 239 Genesis Release Candidate Release Decision CLI commands.
        sprint_239_commands = {
        ...
        if raw_args and raw_args[0] in sprint_239_commands:
            from aura.partner_runtime import (
        ...
        if raw_args and raw_args[0] in sprint_237_commands:
            from aura.partner_runtime import (
    ...
    def run(self, args: list[str] | None = None) -> bool:
        ...
        if handle_localhost_ssh_tunnel_security_review_command(args):
            return True
        if handle_permission_expiry_recovery_review_command(args):
            return True
        if handle_backup_restore_rehearsal_command(args):
            return True
        if handle_aura_launcher_service_controls_command(args):
            return True
        ...
        if handle_local_model_router_activation_command(args):
            return True
        if handle_model_lifecycle_queue_budget_command(args):
            return True
        if handle_active_local_runtime_integration_command(args):
            return True

        if handle_roadmap_reconfirmation_after_v1_2_0_command(args):
            return True
